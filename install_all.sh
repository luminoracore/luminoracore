...
if [ "$success" = true ]; then
    ...
    echo -e "${GRAY}1. Setup v1.1 database: ./scripts/setup-v1_1-database.sh${NC}"
    echo -e "${GRAY}2. Verify installation: python verify_installation.py${NC}"
    echo -e "${GRAY}3. Test v1.1 features: ./scripts/test-v1_1-features.sh${NC}"
    echo -e "${GRAY}4. Configure API keys if needed${NC}"
    echo -e "${GRAY}5. Test: luminoracore --help${NC}"
    echo -e "${GRAY}6. Test v1.1: python examples/v1_1_quick_example.py${NC}"
    echo -e "${GRAY}7. Read: QUICK_START.md and mejoras_v1.1/QUICK_START_V1_1.md${NC}"
else
