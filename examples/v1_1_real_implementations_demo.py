...
- SQLite, PostgreSQL, MySQL, MongoDB, Redis, DynamoDB storage
...
async def main():
    ...
    print(f"✅ Saved {len(facts)} facts to SQLite database")
    print(f"✅ Saved {len(episodes)} episodes to SQLite database")
    facts_str = [f'{f["category"]}:{f["key"]}={f["value"]}' for f in facts]
    episodes_str = [f'{e["type"]}:{e["title"]}' for e in episodes]
