...
def create_storage(
    ...
    """
    Create a storage backend from configuration with optional optimization.
    ...
    Returns:
        Storage backend instance (potentially wrapped with optimizer)
    ...
    if config.storage_type == StorageType.MEMORY:
        ...
        storage = JSONFileStorage(config)
    ...
    if optimizer:
        logger.info(f"Wrapping {config.storage_type} storage with optimizer")
        storage = OptimizedStorageWrapper(storage, optimizer)
    
    return storage


class OptimizedStorageWrapper(SessionStorage):
    """
    ...
    def __init__(self, storage: SessionStorage, optimizer: Any):
        """
        Initialize wrapper
        ...
        """
        # Initialize with storage's config
    ...
    # Delegate all SessionStorage methods to base storage
    # Override only methods that need optimization
    
    async def save_session(self, session_id: str, session_data: Dict[str, Any]) -> bool:
        """Save session - optimization applied if enabled"""
        if self._optimizer and (
            self._optimizer.config.key_abbreviation or
            self._optimizer.config.compact_format or
            self._optimizer.config.minify_json
        ):
            # Compress session data before saving
            compressed = self._optimizer.compress(session_data)
            return await self._storage.save_session(session_id, compressed)
        else:
            return await self._storage.save_session(session_id, session_data)
    
    async def load_session(self, session_id: str) -> Optional[Dict[str, Any]]:
        """Load session - auto-expansion applied"""
        ...
        if self._optimizer and (
            ...
            expanded = self._optimizer.expand(session_data)
        else:
            expanded = session_data
    ...
    async def session_exists(self, session_id: str) -> bool:
        return await self._storage.session_exists(session_id)
    
    # Delegate attribute access for any other methods
